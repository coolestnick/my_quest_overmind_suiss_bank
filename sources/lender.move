...
module quest_overmind::lending {
    ...
    //==============================================================================================
    const EInsufficientCollateral: u64 = 1; // Error code for insufficient collateral
    const EInsufficientBorrowedAmount: u64 = 2; // Error code for insufficient borrowed amount
    const EBorrowedAmountCannotBeZero: u64 = 3; // Error code for borrowed amount cannot be zero
    ...
    */
    public fun create_pool<CoinType>(
        _: &mut AdminCap, // Added underscore to silence unused parameter warning
    ...
    */
    public fun deposit<CoinType>(
        ...
        if (table::contains(&state.users, sender)) {
            let user_data = table::borrow_mut(&mut state.users, sender);

            if (table::contains(&user_data.collateral_amount, pool.asset_number)) {
                let amount = table::borrow_mut(&mut user_data.collateral_amount, pool.asset_number);
                *amount = *amount + value;
            } else {
                table::add(&mut user_data.collateral_amount, pool.asset_number, value);
            }
        } else {
            ...
            table::add(&mut state.users, sender, user_data);
        }
    }
    ...
    */
    public fun withdraw<CoinType>(
        ...
        let real_amount_to_withdraw = 0; // Initialize with 0
        ...
        if (table::contains(&state.users, sender)) {
            ...
            if (table::contains(&user_data.collateral_amount, pool.asset_number)) {
                ...
                assert!(*amount >= amount_to_withdraw, EInsufficientCollateral); // Added error code
        ...
        let amount_to_withdraw_balance = balance::split(&mut pool.reserve, real_amount_to_withdraw);
    ...
    */
    public fun borrow<CoinType>(
        ...
        let real_amount_to_borrow = 0; // Initialize with 0

        if (table::contains(&state.users, sender)) {
            let user_data = table::borrow_mut(&mut state.users, sender);

            if (table::contains(&user_data.borrowed_amount, pool.asset_number)) {
                let amount = table::borrow_mut(&mut user_data.borrowed_amount, pool.asset_number);
                *amount = *amount + amount_to_borrow;
            } else {
                table::add(&mut user_data.borrowed_amount, pool.asset_number, amount_to_borrow);
            };

            real_amount_to_borrow = amount_to_borrow;
        };

        let amount_to_borrow_balance = balance::split(&mut pool.reserve, real_amount_to_borrow);
    ...
    public fun repay<CoinType>(
        ...
        if (table::contains(&state.users, sender)) {
            ...
            if (table::contains(&user_data.borrowed_amount, pool.asset_number)) {
                ...
                assert!(*amount >= value, EInsufficientBorrowedAmount); // Added error code
    ...
    public fun calculate_health_factor(
        ...
        if (table::contains(&state.users, user)) {
            ...
            while (i < state.number_of_pools) {
                ...
                if (table::contains(&user_data.collateral_amount, i)) {
                    let amount = table::borrow(&user_data.collateral_amount, i);
                    collateral_total_amount =
                        collateral_total_amount + (*amount / math::pow(10, decimals as u64)) * price;
                };

                if (table::contains(&user_data.borrowed_amount, i)) {
                    let amount = table::borrow(&user_data.borrowed_amount, i);
                    borrowed_total_amount =
                        borrowed_total_amount + (*amount / math::pow(10, decimals as u64)) * price;
                };
        ...
        assert!(borrowed_total_amount != 0, EBorrowedAmountCannotBeZero); // Added error code

        (collateral_total_amount * 80) / borrowed_total_amount // Adjusting for decimal precision
    }
...
module quest_overmind::dummy_oracle {
    ...
    public fun add_new_coin(price: u64, decimals: u8, feed: &mut PriceFeed) {
        vector::push_back(&mut feed.prices, price);
        vector::push_back(&mut feed.decimals, decimals);
    }

    public fun update_price(new_price: u64, coin_number: u64, feed: &mut PriceFeed) {
        let existing_price = vector::borrow_mut(&mut feed.prices, coin_number);
        *existing_price = new_price;
    }

    public fun get_price_and_decimals(coin_number: u64, feed: &PriceFeed): (u64, u8) {
        (
            *vector::borrow(&feed.prices, coin_number),
            *vector::borrow(&feed.decimals, coin_number),
        )
    }
}